...
theme_data <- read.csv('C:\\Users\\rayzc\\OneDrive\\Pictures\\Documents\\theme_data.csv', header = TRUE)
...
letterlinks <- which(letterlinks[['Origin.Place']] == letterlinks[['End.Place']])
for (i in c(1:length(letterlinks[['End.Place']]))){
  if(letterlinks[['End.Place']][i] != letterlinks[['Origin.Place']][i]){ #If there is a letter response to another location
    communicators_direct[counter] <- i
    counter = counter + 1
  }
}
communicators_direct #Indexes of letter communicators (direct communicators)
...
print(Gender_letterindeces)
print(Family_letterindeces)
...
sum(theme_data[Gender_letterindeces,33:61])
sum(theme_data[Family_letterindeces,33:61])
fulldata <- read.csv('C:\\Users\\rayzc\\OneDrive\\Pictures\\Documents\\bytedatavis.csv', header=TRUE)
crit_data = fulldata[fulldata['Criticism.of.Byte'] == 1, ]
sum(crit_data['Praise.for.Byte'])
dim(crit_data)
#Segment the Data
gendata <- fulldata[Gender_letterindeces,]
famdata <- fulldata[Family_letterindeces,]
gendataONLY <- fulldata[unique(Gender_letterindeces[!(Gender_letterindeces%in%GenderFamily_letterindeces)]),]
famdataONLY <- fulldata[unique(Family_letterindeces[!(Family_letterindeces%in%GenderFamily_letterindeces)]),]
genfamdata <- fulldata[unique(Gender_letterindeces[Gender_letterindeces%in%Family_letterindeces]),]
genfamthemedata <- genfamdata[,33:61]
genthemedata <- gendata[,33:61]
famthemedata <- famdata[,33:61]
genONLYthemedata <- gendataONLY[,33:61]
famONLYthemedata <- famdataONLY[,33:61]

genfamthemedetails <- genfamdata[,c(4,7,13,18,20,21)]
genthemedetails <- gendata[,c(4,7,13,18,20,21)]
famthemedetails <- famdata[,c(4,7,13,18,20,21)]
genONLYthemedetails <- gendataONLY[,c(4,7,13,18,20,21)]
famONLYthemedetails <- famdataONLY[,c(4,7,13,18,20,21)]

#Run one set of these
...
theme_data <- read.csv('C:\\Users\\rayzc\\OneDrive\\Pictures\\Documents\\bytedatavis.csv', header = TRUE)
letter_links <- theme_data[theme_data['In.Reply.To..Full.Letter.Code'] != '',] #Isolate Letters that are in response to atleast one other letter
print(paste0("Number of letters that are referencing at least one other letter: ", length(letter_links['In.Reply.To..Full.Letter.Code',])))
...
#Save the Indexes of these letters that are referencing EXACTLY two or more other letters
#Analyze the associated themes to those letter(s) that are: (1) Doing the 'referencing'/'replying' (2) The original letter of source

#Make correlation matrix from co-occurance matrix
m <- as.matrix(m)
print(m) #This doesnt work for letter replies for some reason...
library("Hmisc")
cor.matrix <- rcorr(m , "pearson")
...
#Inferred Relationship by comparison of two clustering methods:
#Family/Children/Spouse and Gender/Race (Clustered together in both - OWN DISTINCT CLUSTER)
#Novice and Request Byte (Clustered together in both)
#Announcment for Byte Readers and Commentary/Opinion (Clustered together in both)
#Consumer Retail and Personal Finance (Clustered together in both)
#Correction and SourceCode (Clustered together in both)
#Consumer Education and Work Finance (Clustered together in both)
#Praise for Byte and Criticism of Byte (Clutered together in ave - clustered 1 level apart in comp)
